...
export { default as ResponsiveBands } from '../mixins/responsiveBands'
...
export class MediaQueries {
    ...
    between(...args) {
        ...
        return this.expr(`(min-${measurement}: ${this.prepare(minVal)}) and (max-${measurement}: ${this.prepare(maxVal)})`)
    }

    beyond(...args) {
        let [value, measurement = 'width'] = this.getArgs(args)
        let [minVal, maxVal] = value
        return this.expr(`(min-${measurement}: ${this.prepare(maxVal)}), (max-${measurement}: ${this.prepare(minVal)})`)
    }
